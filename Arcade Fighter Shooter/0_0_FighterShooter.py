...
class player:
    ...
    # when up key pressed, y-coordinate -1 for as long as the key is pressed.
        ...
        if is_key_pressed["right"] == True and self.position[0] <= screenwidth:
            self.position[0] += self.speed
    def gun(self):
        #bullets([100,100],(0,0,0),20,1).shoot() #doesnt move because it will alwas be set to 100,100
        bullets(self.position,(0,0,0),20,1).shoot() # changes player balls position because self.position is inherited by bullet
    def all_player_functions(self):
        self.draw()
        self.player_movement_controller()
        self.gun()
        

class bullets:
    def __init__(self,position,color,width,speed):
        self.position = position
        self.color = color
        self.width = width
        self.speed = speed
