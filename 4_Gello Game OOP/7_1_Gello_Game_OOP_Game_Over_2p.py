...
is_key_pressed = {"up" : False,"down" : False,"left" : False,"right" : False,"w":False,"s":False,"a":False,"d":False}
finished = False
start = False
main_menu = True
game_over = False
...
class player:
    def __init__(self,name,position,color,width,speed,keys):
        self.name = name
        self.position = position
        self.color = color
        self.width = width
    ...
    # when up key pressed, y-coordinate -1 for as long as the key is pressed.
        if is_key_pressed["up"] == True or is_key_pressed["w"] == True and self.position[1]>=0:
            self.position[1] -= self.speed
        if is_key_pressed["down"] == True or is_key_pressed["s"] == True and self.position[1] <= screenheight:
            self.position[1] += self.speed
        if is_key_pressed["left"] == True or is_key_pressed["a"] == True and self.position[0]>=0:
            self.position[0] -= self.speed
        if is_key_pressed["right"] == True or is_key_pressed["d"] == True and self.position[0] <= screenwidth:
            self.position[0] += self.speed
...
p1_ball_width = 20
p1_ball_speed = 3
p1_keys = ["up","down","left","right"]
p1 = player("p1",p1_ball_pos,p1_ball_color,p1_ball_width,p1_ball_speed,p1_keys)
#player
p2_ball_pos=[60,100]
p2_ball_color=(0,20,10)
p2_ball_width = 20
p2_ball_speed = 3
p2_keys = ["w","s","a","d"]
p2 = player("p2",p2_ball_pos,p2_ball_color,p2_ball_width,p2_ball_speed,p2_keys)
#
